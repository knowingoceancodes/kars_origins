...
#battle(human vs animal model)

def anim_battle(hero, anim, amr):
    bheal = healing(hero, 50)
    strongheal = healing(hero, 80)
    ultraheal = healing(hero, 200)
    ...
    while hero.is_alive() and anim.health > 0:
        i = input(
            f"\n[1] Fight  |  [2] Block  |  [3] Parry  |  [4] Instinct\n"
            f"{hero.name} HP: {hero.health}  |  {anim.name} HP: {anim.health}\n:~: "
        )

        r = False  # reset each round

        if i == "1":
            hero.attack(anim)

        elif i == "2":
            incoming = random.randint(anim.attack_dmg, anim.attack_dmg + 10)
            r = hero.block(anim, incoming)
            ...
            if not r:
                anim.rip(hero)
                amr.durability -= 5
                cprint(f"{hero.name} was ripped apart by {anim.name}!")
                cprint(f"{hero.name}'s health is now {hero.health}")

        elif i == "4" and hero.health <= 40:  # instinct only unlocks when low HP
            hero.instinct(anim)
            echance = random.randint(1, 100)
            if echance <= 30:
                print(f"{anim.name} prepares a CRITICAL counter!")
            else:
                print(f"{anim.name} prepares a normal counter!")

        # Enemy's turn
        if hero.is_alive() and anim.health > 0:
            if not r:  # player failed block/parry
                echance = random.randint(1, 100)
                if echance <= 30:
                    cprint("Enemy lands a CRITICAL strike!")
                    anim.rip(hero)
                    amr.durability -= 10
                else:
                    anim.rip(hero)
                    amr.durability -= 5
            else:  # successful block/parry
                echance = random.randint(1, 100)
                if echance <= 30:
                    cprint(f"{anim.name} hesitates after failed crit attempt...")
                else:
                    cprint(f"{anim.name} skips turn")
            # Fire breath chance
                if echance <= 10:
                  anim.fire_breath(hero)
        #check fire breath chance 

        # Fatality QTE if enemy is low
        if anim.health <= 30 and hero.is_alive():
            f = input("\nEnter 'f' to conduct FATALITY: ")
            if f.lower() == "f":
                hero.fatality(anim)
            else:
    ...
    # Healing drop
    if rheal and hero.is_alive():
        cprint(f"{hero.name} obtained a potion healing {rheal.power} HP!")
        rheal.heal(hero)
    else:
        cprint(f"{anim.name} dropped nothing.")
...
#-------game world --------#
hero = character("KARS" , 100 , 12 , "I am worse than death" )
guard = character("GUARD" , 100 , 10 , "Your blood will wash away your father's sins!")
...
cprint("-knowing ocean arts presents-")
...
cprint("game intended for mature audiences , includes :- self harm , gore , injury detail , violence ")
ts(2)
...
cprint("\n --kars : the blade of olympus--")

ts(2)

cprint("\n ---this is a demo and doesn't represent the final game---")
ts(2)
cprint("\n -enjoy the demo--")
ts(2)
...
ts(2)
cprint("The guard draws his blade. The crowd watches, waiting for one to fall.")
ts(2)
print()
...
ts(2)
cprint(" MERCY ! MERCY ! - they shout , but the moster within you knew your destiny. ")
cprint("the guard lied lifeless , the floor drank the blood of your enemy.. ")
...
#branched choice
     ...
     #skips a fight with a demon monster
...
if not key: # if player chose canon choice
     ...
     cprint("it wasn't a lion , it wasn't a hyena , it was something unknown.. unheard..")

     cprint("you steady yourselves as the beast approaches you...")

     demon = animal("the beast" , 15 , 200 , "*growl*")

     anim_battle(hero , demon , arm1 )
...
cprint(''' \n
          ...
          |                         kars: origins                              |
          |                                                                    |
          |                   programmer:- DIVYANSHU PANDA                     |       
          |                   storyboard:- DIVYANSHU PANDA                     |
          |                                                                    |
...
if beast_faught:
     cprint("\n-YOUR BLOOD BURNS THE SHAKLES TURN INTO ASHES-")
     sp()
     cprint("\nKARS : Incinerate !")
     lp()
